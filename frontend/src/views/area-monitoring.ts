 ...
 * quantity statistics reconcile — Areas with Machines:
 * `Total pcs = Queued + On machines + Done`; Areas without Machines:
 * `Total pcs = Processing + Done`; the terminal Stockroom shows
 * stocked totals instead. Hot renders `—` when zero; the reconciling
 * quantity values always render as numbers.
 */
