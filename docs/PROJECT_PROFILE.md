...
- a separate Retired Machines table with sortable data columns: name, `Retired on YYYY-MM-DD`, asset metadata, notes — historical display and reporting plus a per-row **Reactivate** action (§8.6 return to service); retired Machines still never appear in assignment choices and accept no new scans,
