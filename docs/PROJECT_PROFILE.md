...
**Badge confirmation of sensitive actions (v19):** each of the three sensitive Scan Station actions — `DONE` (Area completion), `QUEUE` (return unfinished quantity to queue) and Undo — carries its own **badge-confirmation option** in the Worker sessions policy (Administration → Worker sessions), default **enabled**:
...
- in a **fixed-Worker** Area a **final confirmation question** restating the key facts appears instead (no badge exists there) — the plain presentation for DONE, a warning presentation for QUEUE return and Undo;
- Areas without Worker identification, and scanned-session Areas with the option disabled, keep the plain confirmation step;
