...
- **PLANNED Quantity Flow**: the `expected_duration` of the **current** Assigned Route Step when it has a value; otherwise the `default_expected_duration` of the Operation recorded on the current effective position. The current step is derived from immutable Movement history, never from Area equality: the arrival that established the current position (`RECEIVED`, `TRANSFERRED`) either fulfilled a route step — the quantity is **on route** at that step — or was a confirmed deviation referencing no step — the quantity is **off route** and has no current step until a later arrival fulfils one. A deviation back into the Area of an earlier step (a Repair return) does not make that step current again: the route still expects its next step. In-Area events (Machine assignment, release, `DONE`) keep the state their arrival set; an undone arrival never happened; a split child or merge result inherits the state of its source(s). Off-route quantity therefore takes the Operation default.
