...
- in a **fixed-Worker or disabled-Worker** Area the same option gates a **final confirmation question** restating the key facts instead (no badge exists there) — an information-toned presentation for DONE, a warning presentation for QUEUE return and Undo;
- an action whose option is disabled keeps the plain confirmation step in every Area;
