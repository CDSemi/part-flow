...
Implementation state: the backend of this phase is implemented and audited (see Current State) — migration `0006_phase5_transfer`, the transfer command and the Scan Station read models under `/api/scan-stations/{station_id}/…` and `/api/areas/{area_id}/inventory`. Route deviation — Area or Operation — is confirmed explicitly with a reason and recorded on the `TRANSFERRED` Movement itself; the separate deviation/route-adjustment events and route editing belong to route editing (Phase 9+). Remaining for this phase: the real Scan Station view wired to this API (keyboard focus restoration, Last Scanned PN, and the inventory refresh are presentation behavior of that wiring).
