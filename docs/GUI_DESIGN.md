...
> An interactive mockup accompanies this document: `mockups/partflow-gui-mockup-v12.html`.
> Supersedes GUI Design v11 (mockup: `archive/partflow-gui-mockup-v11.html`); the differences are listed in §12.1. Differences from v10, v9, v8, v7, v6, v5, v4, v3, v2 and v1 remain listed in §12.2–§12.11.
...
- The toggle lives in the top navigation bar and applies to the whole application instantly, including navigation chrome, dialogs and toasts. No view has a fixed theme anymore. In Scan Station **production mode** — where the top navigation is hidden — a compact variant of the same shared control renders in the station header's actions group beside the connectivity chip (§4.1), using the same global theme state (`aria-pressed`, visible focus state); the selected theme is preserved when switching between standard and production routes.
...
Every quantity view uses one fixed visual order: **PN → selected context or instruction → guidance/validation message → quantity input → numpad → navigation buttons**. The guidance sits directly above the input as semantic text (never inside a framed or dashed summary box, never a heavy card): the **kinds are visually distinct (v12)** — neutral instructions stay plain muted text (`Enter the physical quantity received. No default quantity is assumed.`); information, important constraints, required actions and validation errors each carry a small marker plus a thin accent edge in their semantic tone (`ℹ` info, `⚠` warning, `›` required action, `✕` validation error), so color is never the only distinction and validation reads strongest (`Quantity cannot exceed the 6 pcs currently available at the source.`). The validation message updates next to the input, entered values are preserved, and **Next stays disabled while the quantity is invalid**.
...
- **Header clock (v7, refined v12):** the header shows the live local date **and time**, driven by a self-updating one-second clock component — never a static mock timestamp. The clock reads as **one coherent block**: the time is the strongest element, with the date directly beneath it as a clearly associated secondary line; plain text on the header surface (no border, no background) so it never resembles an editable control, sized for large shared displays in both themes.
...
- **Footer (restructured v12):** the footer is a normal flex child **anchored to the bottom of the board viewport** through the board's flex layout (`margin-top: auto`) — never `position: fixed`, never covering table content, its real height always included in the dynamic pagination measurement, and empty/one-page states still place it at the bottom without creating a scrollbar. Its content is organized into readable groups instead of one undifferentiated sentence: a controls row (page indicator, manual navigation, aggregate stats — active PNs, pcs in production, pcs stocked, pcs scrapped) and a legend row (flame, blink, `—` conventions) that ends with the user-facing **sorting rule**: `Order: Hot rank first → earliest due date → no due date by oldest received date.` — all three levels stated, deterministic tie-breakers and implementation field names never exposed.
...
Management sub view (follows the global theme mode, §2.1) implementing manual Work Order entry and explicit production release (PROJECT_PROFILE §13; §21 *Work Orders* — the view was called *PO Intake*, then *Purchase Orders*, before the v6 vocabulary migration, §12.7). It handles business demand only — it is not ERP-style customer, pricing, invoicing, shipping, purchasing, or accounting functionality.
...
> Historical entries in §12.6–§12.11 intentionally keep the vocabulary of the versions they describe (REWORK, temporary `TMP-…` Work Order Numbers, Machine sessions, Purchase Order / PO / PO Demand / PO Intake). Since v6 the canonical term is **Work Order** (§12.6 item 9); since v8 REWORK, temporary Work Order Numbers, Machine sessions, Action barcodes, and the Recent Scans list no longer exist — the older names appear below only as history.

## 12.1 Changes from GUI Design v11
...
## 12.2 Changes from GUI Design v10
...
## 12.3 Changes from GUI Design v9
...
## 12.4 Changes from GUI Design v8
...
## 12.5 Changes from GUI Design v7
...
## 12.6 Changes from GUI Design v6
...
## 12.7 Changes from GUI Design v5
...
## 12.8 Changes from GUI Design v4
...
## 12.9 Changes from GUI Design v3
...
## 12.10 Changes from GUI Design v2
...
## 12.11 Changes from GUI Design v1
