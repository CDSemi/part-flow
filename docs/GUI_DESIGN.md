...
> An interactive mockup accompanies this document: `mockups/partflow-gui-mockup-v10.html`.
> Supersedes GUI Design v9 (mockup: `archive/partflow-gui-mockup-v9.html`); the differences are listed in §12.1. Differences from v8, v7, v6, v5, v4, v3, v2 and v1 remain listed in §12.2–§12.9.
...
10. **Professional operator copy.** Instructions and descriptions explain what the user should do, what information is required, what will happen after confirmation, and why an action is unavailable or rejected. Normal operator guidance never exposes raw implementation constants or enum combinations (such as `(QUANTITY_ADJUSTED · INCREASE)` or `movement_reason = REPAIR` as guidance text); canonical Movement types may still appear where they are actual audit/history data — the Movement history, type badges, and the confirmation summary's `Recorded event(s)` row. Cancel buttons throughout the workflows are exactly `Cancel (Esc)` — never long suffixed labels. Development-only demo barcode instructions stay clearly development-only and are excluded from production builds; repetitive "(mock)" / "nothing persisted" phrasing is reduced to at most one concise development note per surface.
...
- `/scan-station` — the **Station Selector**. It never auto-redirects to a station. It lists the active Scan Stations with enough information to distinguish them: Station ID, Department, Area (with color), supported Operations, and whether the Area has Machines.
- `/scan-station/:stationId` — one station (e.g. `/scan-station/LATHE-ST-01`). An unknown or inactive Station ID shows a clear error and never silently falls back to another station.

The footer keeps the faint Station ID caption; it is **subtly clickable** and opens the Station Selector for switching stations — an intentionally unobtrusive affordance, not a promoted operator workflow.
...
- **Part Number** renders on a single line — the column carries an explicit minimum usable width (applied to the header/cell content, not only `<col>` behavior) so the Hot indicator and a standard full PN such as `2027-60-8114-00` never clip or wrap; longer valid PNs may take additional width or controlled overflow. Name and revision appear as a secondary line. After every column has enough width for its content, remaining table width is distributed primarily among Part Number, Areas & Quantities · Time, and Due Date — never forced entirely into Job Numbers; No. and Total Days stay compact.
...
- **Areas & Quantities · Time** uses a **stable grid** (v7) with **explicit presentation data** (v10): each location row carries the Area identity/label, an optional separate Machine name, quantity, state, and time — Area and Machine are never combined into one opaque display string and Machine names are never parsed back out of formatted text. Actively Machine-assigned quantity renders as `[Area color dot] [Lathe 2 chip]  4  on mch.  1h 05m`: the Machine name in a compact rounded chip (label look — subtle chip/panel background, subtle border, rounded corners, both themes, no action color, no excessive padding; long names truncate within a sensible maximum width with the full name in `title`), the exact concise state `on mch.` (never the standalone word `machine`), quantity and time fixed and readable, nothing overlapping. Queued quantity keeps a `queue` label; direct processing shows `processing`. DONE/`READY_TO_TRANSFER` quantity is visually distinct from active Machine processing: it shows the current Area with a concise `done` state, never `on mch.`, never the Machine as current executor — a Machine that completed the work appears only as secondary completion context (tooltip). Times stay right-aligned in monospace; a time turns amber when it exceeds the expected duration of the active Route Step. A dashed separator shows the PN total underneath.
...
- **Scrap visibility:** a PN with scrapped quantity shows a quiet `⊘ n scrapped` value in its **own clearly separated layout space** — a dedicated line beneath the total row, never inside the normal time track and never overlapping other fields, readable with long Area/Machine names; the footer legend explains the symbol and carries the Department scrap total.
...
- **Reorder requires confirmation (v7), presented as a structured comparison (v10):** every operation that changes the order of existing Hot entries — drag-and-drop (visible grip), Move Up, Move Down, Undo, and Redo — opens a **confirmation dialog before applying**. The dialog leads with a primary summary of the item the user moved (`Move 2027-60-8114-00 · WO 007010 from #4 to #2` plus a computed secondary line such as `2 other demands will shift down.`), followed by a compact current-versus-proposed comparison (columns Work Order Demand | Current | New) covering the affected entries: the directly moved item is highlighted, indirectly shifted entries render with less emphasis, and ↑/↓ indicators make the direction visible next to old and new ranks. Buttons: `Apply ranking` and `Cancel (Esc)`. Cancel leaves the list order and both Undo/Redo histories unchanged; the visible list is **never renumbered before confirmation**. Undo and Redo confirmations use the user-facing titles `Restore previous ranking` and `Reapply ranking` (the implementation action name is secondary detail only) and present the same comparison. Adding a new entry at the bottom keeps its direct behavior (no order-change confirmation).
...
Management sub view (follows the global theme mode, §2.1) implementing manual Work Order entry and explicit production release (PROJECT_PROFILE §13; §21 *Work Orders* — the view was called *PO Intake*, then *Purchase Orders*, before the v6 vocabulary migration, §12.5). It handles business demand only — it is not ERP-style customer, pricing, invoicing, shipping, purchasing, or accounting functionality.
...
## 12.2 Changes from GUI Design v8
...
## 12.3 Changes from GUI Design v7
...
## 12.4 Changes from GUI Design v6
...
## 12.5 Changes from GUI Design v5
...
## 12.6 Changes from GUI Design v4
...
## 12.7 Changes from GUI Design v3
...
## 12.8 Changes from GUI Design v2
...
## 12.9 Changes from GUI Design v1
